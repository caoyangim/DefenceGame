...
export const TOWER_STATS: Record<TowerType, Omit<Tower, 'id' | 'x' | 'y' | 'cooldown' | 'upgrades'>> = {
  [TowerType.BASIC]: {
    ...
    damage: 15,
    cooldownMax: 30, // Frames (assuming 60fps)
    cost: 50,
    color: '#3b82f6' // Blue
  },
  [TowerType.SNIPER]: {
...
export const ENEMY_STATS: Record<EnemyType, { hp: number; speed: number; reward: number; color: string }> = {
  [EnemyType.GOBLIN]: { hp: 30, speed: 0.05, reward: 5, color: '#22c55e' }, // Green
  [EnemyType.ORC]: { hp: 80, speed: 0.03, reward: 12, color: '#f97316' }, // Orange
  [EnemyType.TANK]: { hp: 200, speed: 0.015, reward: 25, color: '#64748b' }, // Slate
  [EnemyType.BOSS]: { hp: 1000, speed: 0.01, reward: 100, color: '#a855f7' } // Purple
};
...
export const getWaveConfig = (wave: number): WaveConfig[] => {
  ...
  // Complexity ramps up with waves
  if (wave === 1) {
